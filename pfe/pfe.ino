...
int get_wind_direction()
{
  ...
  if (adc ==958) return (113);
    if (adc == 940) return (68);
    if (adc == 931) return (90);
    if (adc == 897) return (158);
    if (adc == 839) return (135);
    if (adc == 779) return (203);
    if (adc == 737) return (180);
    if (adc == 618) return (23);
    if (adc == 563) return (45);
    if (adc == 425) return (248);
    if (adc == 394) return (225);
    if (adc == 321) return (338);
    if (adc == 238) return (0);
    if (adc == 196) return (293);
    if (adc == 137) return (315);
    if (adc == 79) return (270);
...
void printWeather()
{
   ...
   if(winddir==248)Serial.print("north-northwest wind (NNW)");
    if(winddir==270)Serial.println(",  west wind (W)");
   if(winddir==293)Serial.println(",  west-northwest wind (WNW)");
    if(winddir==315)Serial.println(",  northwest wind (NW)");
    if(winddir==338)Serial.println(",  north-northwest wind (NNW)");*/
    Serial.print(adc);//winddir
    //winddirwrirte(winddir);
...
/*void winddirwrirte(char variables){
if(variables==0||variables==360)Serial.println(",  north wind (N)");
    if(variables==23)Serial.println(",  north-northeast wind (NNE)");
    if(variables==45)Serial.println(",  northeast wind (NE)");
    if(variables==68)Serial.println(",  east-northeast wind (ENE)");
    if(variables==90)Serial.println(",   east wind (E)");
    if(variables==113)Serial.println(",   east-southeast wind (ESE)");
    if(variables==135)Serial.println(",   southeast wind (SE)");
    if(variables==158)Serial.println(", south-southeast wind (SSE)");
    if(variables==180)Serial.println(",  south wind (S)");
    if(variables==203)Serial.println(",  south-southwest wind (SSW)");
    if(variables>=220&&variables<=230)Serial.println(",  southwest wind (SW)");
   if(variables==248)Serial.println(", west-southwest wind (WSW)");
    if(variables==270)Serial.println(",  west wind (W)");
   if(variables==293)Serial.println(",  west-northwest wind (WNW)");
    if(variables==315)Serial.println(",  northwest wind (NW)");
    if(variables==338)Serial.println(",  north-northwest wind (NNW)");
