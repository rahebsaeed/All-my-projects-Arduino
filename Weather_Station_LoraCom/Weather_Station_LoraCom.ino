...
#define TX_INTERVAL 3000   // 3 second between transmissions (adjust as needed)
#define sipeedSerial Serial2

// Hardware Serial for RAK811 (use SoftwareSerial if needed)
#define RAK811_RX 15
#define RAK811_TX 14
#define DebugSerial Serial

SoftwareSerial loraSerial(RAK811_RX, RAK811_TX);  // RX, TX
...
void setup() {
  Serial.begin(115200);
  // Vitesse très rapide de 2,000,000 bauds, comme vous l'avez découvert.
  sipeedSerial.begin(2000000);
  loraSerial.begin(115200);
  while (!Serial) { ; }
  delay(2000);

  Serial.println("Sending join command: at+join");
...
void loop() {
  ...
  if (millis() - lastSecond >= 1000) {
    ...
    // mesure csensors soil and temperature soil
    readSensors();
...
void readSensors() {
  ...
  int rawHumidity = analogRead(FC28_PIN);
  soilHumidity = constrain(map(rawHumidity, SEC_VALUE, WET_VALUE, 0, 100), 0, 100);
...
void HeightRead() {
  // S'il y a des données qui arrivent depuis le Sipeed...
  if (sipeedSerial.available() > 0) {
    // ...on lit la ligne entière jusqu'au caractère de fin de ligne ('\n')...
    message = sipeedSerial.readStringUntil('\n');

    // ...et on la retransmet immédiatement au PC.
    // On ne retransmet que les lignes qui contiennent bien nos données pour éviter le "bruit".
    int sepIndex = message.indexOf(':');

    // Extract the substring after ':'
    message = message.substring(sepIndex + 1);
    Serial.println("hauteur:"+ message);
  }
}
void sendLoRaData() {
  // 1. Get fresh measurements
  calcWeather();
  Serial.println("--- Preparing to Send LoRa Data ---");

  // 2. Prepare data (same scaling as before)
  uint16_t windDir = winddir;
  uint16_t windSpeed_x10 = windspeedmph * 1.60934 * 10;       // km/h * 10
  int16_t temperature_x10 = ((tempf - 32) * 5.0 / 9.0) * 10;  // Celsius * 10
  uint8_t humidity_x2 = humidity * 2;                         // % * 2
  uint16_t rainfall_x10 = rainin * 10;                        // mm * 10
  uint16_t pressure_hPa = pressure / 100;                     // hPa

  // 3. Create ASCII payload (JSON-like format)
  String payload = "{";
  payload += "\"wd\":" + String(windDir) + ",";
  payload += "\"ws\":" + String(windSpeed_x10) + ",";
  payload += "\"tp\":" + String(temperature_x10) + ",";
  payload += "\"hm\":" + String(humidity_x2) + ",";
  payload += "\"rn\":" + String(rainfall_x10) + ",";
  payload += "\"pr\":" + String(pressure_hPa) + ",";
  payload += "\"tm\":" + String(temperature * 10, 0) + ",";
  payload += "\"hs\":" + String(soilHumidity) + ",";
  payload += "\"ht\":" + String(message);

  payload += "}";
  Serial.print("  Longth of data in bytes in payload: ");
  Serial.println(payload.length());

  // 4. Debug output
  Serial.print("  ASCII Payload: ");
  Serial.println(payload);

  // 5. Send as hex-encoded ASCII (required by RAK811)
  String hexPayload = "";
  for (int i = 0; i < payload.length(); i++) {
    char c = payload[i];
    if (c < 0x10) hexPayload += "0";  // Pad single digits
    hexPayload += String(c, HEX);
  }
  Serial.print("  Longth of data in bytes in Hex: ");
  Serial.println(hexPayload.length());

  String command = "at+send=lora:1:" + hexPayload;
  Serial.print("  Sending Command: ");
  Serial.println(command);

  // 6. Send the command
  RAKLoRa.sendRawCommand(command);
