...
void loop() {
  ...
  if(Distance_F < MIN_OBSTACLE_DISTANCE ){
    if(Distance_R > Distance_L ){
      while(sonarFront.ping_cm() < MIN_OBSTACLE_DISTANCE && digitalRead(INF_R) != IR_OBSTACLE_STATE){
        turnRight(TURN_SPEED);
        delay(200);
      }
      if(digitalRead(INF_R) == IR_OBSTACLE_STATE && sonarFront.ping_cm() < MIN_OBSTACLE_DISTANCE || digitalRead(INF_L) == IR_OBSTACLE_STATE && digitalRead(INF_R) == IR_OBSTACLE_STATE && sonarFront.ping_cm() < MIN_OBSTACLE_DISTANCE){
        if(digitalRead(INF_R) == IR_OBSTACLE_STATE){
          while(digitalRead(INF_R) == IR_OBSTACLE_STATE && digitalRead(INF_B) != IR_OBSTACLE_STATE){
              moveBack(BACKUP_SPEED);
            }
        }else if (digitalRead(INF_L) == IR_OBSTACLE_STATE){
          while(digitalRead(INF_L) == IR_OBSTACLE_STATE && digitalRead(INF_B) != IR_OBSTACLE_STATE){
              moveBack(BACKUP_SPEED);
            }
        }
      }
    }else{
      while(sonarFront.ping_cm() < MIN_OBSTACLE_DISTANCE && digitalRead(INF_L) != IR_OBSTACLE_STATE){
        turnLeft(TURN_SPEED);
        delay(200);
      }
            if(digitalRead(INF_R) == IR_OBSTACLE_STATE && sonarFront.ping_cm() < MIN_OBSTACLE_DISTANCE || digitalRead(INF_L) == IR_OBSTACLE_STATE && digitalRead(INF_R) == IR_OBSTACLE_STATE && sonarFront.ping_cm() < MIN_OBSTACLE_DISTANCE){
    ...
    if(digitalRead(INF_R) == IR_OBSTACLE_STATE && sonarFront.ping_cm() < MIN_OBSTACLE_DISTANCE){

    }
  }else {
    if(irRight != IR_OBSTACLE_STATE && irLeft != IR_OBSTACLE_STATE){
      ...
      if(irRight == IR_OBSTACLE_STATE){
        if(irLeft == IR_OBSTACLE_STATE){
          if(irRight == IR_OBSTACLE_STATE && irLeft == IR_OBSTACLE_STATE && irBack != IR_OBSTACLE_STATE){
            while((digitalRead(INF_R) == IR_OBSTACLE_STATE && digitalRead(INF_L) == IR_OBSTACLE_STATE) && digitalRead(INF_B) != IR_OBSTACLE_STATE){
              moveBack(BACKUP_SPEED);
            }
          }
          // here that we will add right and left ultrasonic sensors

        }else{
          if(irRight == IR_OBSTACLE_STATE && irBack != IR_OBSTACLE_STATE){
            while(digitalRead(INF_R) == IR_OBSTACLE_STATE && digitalRead(INF_B) != IR_OBSTACLE_STATE){
              moveBack(BACKUP_SPEED);
            }
          }
      ...
      }else if(irLeft == IR_OBSTACLE_STATE){
        if(irLeft != IR_OBSTACLE_STATE && irBack != IR_OBSTACLE_STATE){
          while((digitalRead(INF_L) == IR_OBSTACLE_STATE) && digitalRead(INF_B) != IR_OBSTACLE_STATE){
            moveBack(BACKUP_SPEED);
          }
        }
