...
#define WALL_DETECTION_THRESHOLD 7  // cm (consider wall present)
#define DEADBAND 2                  // cm (reduced from 3cm)
#define TARGET_OFFSET 5      // 7cm from each wall
...
void distance() {
  ...
  if (Distance_F < 20 && Distance_F != 0) {
    ...
    // Case 1.1: Both sides are blocked (<10cm)
    if (Distance_R < 10 && Distance_L < 10) {
      // Back up until at least one side is clear
      do {
        moveBackward(SLOWLY);
        delay(20);
        Distance_L = sonarLeft.ping_cm();
        Distance_R = sonarRight.ping_cm();
      } while (Distance_L < 15 && Distance_R < 15);  // Back up until 15cm clearance
      delay(100);
